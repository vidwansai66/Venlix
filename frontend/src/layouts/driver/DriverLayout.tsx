...
export const DriverLayout = () => {
  const [sidebarOpen, setSidebarOpen] = useState(() => {
    const saved = localStorage.getItem('venlix-driver-sidebar');
    return saved !== null ? JSON.parse(saved) : true;
  });

  useEffect(() => {
    localStorage.setItem('venlix-driver-sidebar', JSON.stringify(sidebarOpen));
  }, [sidebarOpen]);
  ...
  return (
    <div className="relative min-h-screen bg-brand-background text-brand-text flex flex-col font-sans transition-colors duration-300">
      ...
      >
        <DriverNavbar sidebarOpen={sidebarOpen} setSidebarOpen={setSidebarOpen} />
