          ...
          >
            <div className="flex items-center justify-between p-5 border-b border-brand-border bg-brand-background/50">
              <h2 className="text-base font-bold text-brand-text flex items-center gap-2">
                <HeadphonesIcon size={18} className="text-primary" /> Contact Support
              </h2>
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="p-6 space-y-4">
              <div>
                <label className="block text-xs font-bold text-muted uppercase tracking-wider mb-2">Subject</label>
                <select 
                  {...register("subject")}
                  className={`w-full px-4 py-3 rounded-xl bg-brand-background border focus:outline-none transition-colors ${errors.subject ? 'border-danger focus:border-danger/80' : 'border-brand-border focus:border-primary/50 text-brand-text'}`}
                >
                ...
                {errors.subject && <p className="text-danger text-xs font-semibold mt-1.5">{errors.subject.message}</p>}
              </div>
              ...
              <div>
                ...
                <textarea
                  {...register("message")}
                  rows={4}
                  className={`w-full px-4 py-3 rounded-xl bg-brand-background border focus:outline-none transition-colors resize-none ${errors.message ? 'border-danger focus:border-danger/80' : 'border-brand-border focus:border-primary/50 text-brand-text'}`}
                  placeholder="Describe your issue..."
                />
                {errors.message && <p className="text-danger text-xs font-semibold mt-1.5">{errors.message.message}</p>}
              </div>
